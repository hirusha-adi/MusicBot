...
class Basic(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            embed.add_field(name="Possible Fix", value=f"use `{config.BotInfo.BOT_PREFIX}help all` to list out all the command and check the proper usage of the command you used", inline=True)
            await ctx.send(embed=embed)
